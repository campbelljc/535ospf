...
void OSPFSendHelloPacket(uchar *dst_ip)
{
	...
	hello_packet_t *hello_pkt = (hello_packet_t *)((uchar *)ospf_pkt + ospf_pkt->message_length*4);
	...
	gpacket_t finished_pkt = OSPFCreateHeader(out_pkt, 1, sizeof(hello_pkt));
...
void OSPFSendLSUPacket(uchar *dst_ip, int seqNum_, uchar* sourceIP)
{
	...
	ospf_hdr_t *ospf_pkt = (ospf_hdr_t *)(out_pkt->data.data);
	ospf_pkt->ospf_message_length = 4;                                 
	lsa_packet_t *lsa_pkt = (lsa_packet_t *)((uchar *)ospf_pkt + ospf_pkt->message_length*4);
...
//	lsu_link_t *currentLink = (lsu_link_t *)((uchar *)lsu_pkt + 1);
	...
	{
		lsu_pkt->links[count].lsu_link_type = ; // 2 for any-to-any, 3 for stub network.
		lsu_pkt->links[count].lsu_link_ID = ; // TODO
		COPY_IP(lsu_pkt->linkscount.lsu_link_data, interfaceIPs[count]);
...
gpacket_t* OSPFSendLSAPacket(gpacket_t *gpkt, int seqNum_, uchar* sourceIP)
{
	...
	lsa_pkt->lsa_age = 0;
	lsa_pkt->lsa_type = 1;
	lsa_pkt->lsa_sequenceNumber = seqNum_;
	lsa_pkt->checksum = 0;
...
{
	opsf_hdr_t *header = (ospf_hdr_t *)pkt->data.data;
